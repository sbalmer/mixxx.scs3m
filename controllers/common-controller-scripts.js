...
bpm.tapButton = function(deck) {
    ...
    if (tapDelta>2.0) { // reset if longer than two seconds between taps
        if (bpm.tap.length > 0) bpm.tap=[];
        return;
    }
    bpm.tap.push(60/tapDelta);
    bpm.tap = bpm.tap.slice(-10);  // Keep the last 8 samples for averaging
print(bpm.tap)
    if (bpm.tap.length > 1) {
        var sum = 0;
...
print("Script: BPM="+average);
        ...
        engine.setValue("[Channel"+deck+"]","rate", (fRateScale - 1)/engine.getValue("[Channel"+deck+"]","rateRange"));
    }
}
